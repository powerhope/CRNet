...
class CatBottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, block_num=3, stride=1):
        ...
        self.conv_list = nn.ModuleList()
        self.stride = stride
#         self.atenchannal2 = ChannelAttention(out_planes)
        if stride == 2:
            self.avd_layer = nn.Sequential(
                nn.Conv2d(out_planes//2, out_planes//2, kernel_size=3, stride=2, padding=1, groups=out_planes//2, bias=False),
                nn.BatchNorm2d(out_planes//2),
...
class STDCNet1446(nn.Module):
    def __init__(self, base=64, layers=[7,8,6], block_num=4, type="cat", num_classes=1000, dropout=0.20, pretrain_model='', use_conv_last=False):
...
#         self.t256 = nn.Sequential(self.features[5])
#         self.addlayer7 = nn.Sequential(self.features[6])
#         self.x16 = nn.Sequential(self.features[7:11])
#         self.t512 = nn.Sequential(self.features[11])
#         self.addlayer14 = nn.Sequential(self.features[12])
#         self.x32 = nn.Sequential(self.features[13:16])
#         self.t1024 = nn.Sequential(self.features[16])
#         self.addlayer19 = nn.Sequential(self.features[17])
